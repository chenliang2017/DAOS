...
struct ABTI_global {
    ...
    ABTI_xstream *p_xstream_head; /* List of ESs (head). The list is sorted(°´rankµÄÐòºÅÉýÐòÅÅÁÐ). */
    ABTD_spinlock
...
struct ABTI_xstream {
    ...
    int rank;                 /* Rank, xstreamçš„ç¼–å·,ä»Ž0å¼€å§‹ */
    ...
    ABTI_ythread
        *p_root_ythread; /* Root thread that schedulers the main scheduler. ä¸»åç¨‹ */
