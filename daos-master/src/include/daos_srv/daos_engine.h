...
struct dss_thread_local_storage {
	uint32_t	dtls_tag;
	void		**dtls_values;	// 数组指针, 元素类型为 struct dss_module_info/struct obj_tls/...
};
...
struct dss_module_key {
	...
	/* The position inside the dss_module_keys, 在dss_module_keys中的索引位置, 便于反向查找 */
	int dmk_index;	
...
extern pthread_key_t dss_tls_key;                  // 线程变量
...
dss_get_module_info(void)
{
	...
	dmi = (struct dss_module_info *)
	      dss_module_key_get(dtc, &daos_srv_modkey);  // 获取daos全局的线程变量
	return dmi;
}
...
struct dss_module {
	/* Name of the module */
	const char		*sm_name;	// 模块名
	/* Module id see enum daos_module_id */
	int				sm_mod_id;	// 模块ID
	/* Module version */
	int				sm_ver;		// 各模块的版本, 由模块各自维护
	...
	int				(*sm_init)(void);		// 加载模块后执行初始化动作
	/* Finalization function, invoked just before module unload */
	int				(*sm_fini)(void);		// 去初始化动作，卸载模块前使用
	...
	struct crt_proto_format		*sm_proto_fmt;		// 反序列化参数，针对不同的Op类型进行操作，生成对应的结构体
	/* The count of RPCs which are dedicated for client nodes only */
	uint32_t			sm_cli_count;				// 支持的Op数量
	/* RPC handler of these RPC, last entry of the array must be empty */
	struct daos_rpc_handler		*sm_handlers;		// CART协议，注册支持的回调函数句柄集
	/* dRPC handlers, for unix socket comm, last entry must be empty */
	struct dss_drpc_handler		*sm_drpc_handlers;	//  dRPC协议，只有mgmt模块注册了该功能，其他模块未使用
