...
struct daos_obj_md {
	daos_obj_id_t	omd_id;			// 对象ID
	uint32_t		omd_ver;		// 来自pool_map的当前版本号
		 ...
		 daos_oclass_id_t cid, uint32_t args)
{
	 ...
	 */
	hdr  = ((uint64_t)OID_FMT_VER << OID_FMT_VER_SHIFT);	// 1 << 60, 4bits
	hdr |= ((uint64_t)ofeats << OID_FMT_FEAT_SHIFT);		// ofeats << 44, 16bits
	hdr |= ((uint64_t)cid << OID_FMT_CLASS_SHIFT);			// cid << 32, 12bits
	oid->hi |= hdr;
}
