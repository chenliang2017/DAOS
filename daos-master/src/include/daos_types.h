 ...
 */
typedef struct {
	uint64_t	lo;		// 低96位, 用户提供, 要求container内唯一
	uint64_t	hi;		// 高32位, 留作daos内部使用: OID_FMT_VER_BITS (version)(4bits) + 
} daos_obj_id_t;		//							 OID_FMT_FEAT_BITS (object features)(16bits) +
						//							 OID_FMT_CLASS_BITS (object class)(12bits)
