...
static int
crt_opc_reg(struct crt_opc_info *opc_info/* 出参 */, crt_opcode_t opc/* Op */, uint32_t flags,
	    struct crt_req_format *crf/* 反序列化参数，针对不同的Op类型进行操作，生成对应的结构体(入参和出参) */, 
	    crt_rpc_cb_t rpc_cb/* Op对应的回调函数 */,
		 ...
		 struct crt_proto_format *cpf)
{
	...
	L3_map = get_L3_map(L2_map, cpf);  // 根据不同协议的版本找子集
	if (L3_map == NULL)
		return -DER_NOMEM;
...
crt_proto_reg_L1(struct crt_opc_map *map, struct crt_proto_format *cpf)
{
	...
	L2_map = &map->com_map[index];  // 根据不同的服务找子集
	D_ASSERT(L2_map != NULL);
