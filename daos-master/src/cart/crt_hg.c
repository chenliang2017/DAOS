...
crt_rpc_handler_common(hg_handle_t hg_hdl)
{
	...
	rc = crt_hg_unpack_header(hg_hdl, &rpc_tmp, &proc);		// 解析出消息类型
	if (unlikely(rc != 0)) {
	 ...
	 */
	rpc_tmp.crp_pub.cr_opc = opc;  // Op类型

	opc_info = crt_opc_lookup(crt_gdata.cg_opc_map, opc, CRT_UNLOCK);	// 从初始化注册的表中找到Op对应的结构：struct crt_opc_info
	...
	rpc_priv->crp_opc_info = opc_info;		// Op关联的操作赋值到结构体：crt_rpc_priv
