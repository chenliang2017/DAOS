...
struct crt_gdata {
	...
	struct crt_opc_map	*cg_opc_map;	// 存放不同模块注册的全部协议
...
struct crt_opc_info {
	...
	crt_rpc_cb_t		 coi_rpc_cb;		// Op对应的回调函数
	...
	struct crt_req_format	*coi_crf;		// Op反序列化参数，针对不同的Op类型进行操作，生成对应的结构体(入参和出参)
};
...
struct crt_opc_map {
	...
	struct crt_opc_map_L2	*com_map;  // 数组, 不同模块的协议集数组(pool/obj/contain...)
};
