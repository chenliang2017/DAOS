...
int
daos_obj_open(daos_handle_t coh /*容器句柄*/, daos_obj_id_t oid /*对象ID*/, unsigned int mode /*对象打开模式*/,
	      daos_handle_t *oh /*对象句柄*/, daos_event_t *ev)
{
	tse_task_t *task;
	int rc;
	...
	return dc_task_schedule(task, true);  // 立即调度task, 在当前线程上下文中执行
}
...
int
daos_obj_update(daos_handle_t oh /*对象句柄*/, daos_handle_t th /*事务句柄*/, uint64_t flags,
		daos_key_t *dkey, unsigned int nr /*akey的数量*/, daos_iod_t *iods,
