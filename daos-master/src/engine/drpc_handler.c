 ...
 * API functions
 * dRPC回调句柄的注册/释放/回调处理
 * 只有mgmt_module模块初始化的时候注册了drpc句柄, 其他模块均为使用, 详见src/mgmt/srv.c : mgmt_module
 */

// 为全局的registry_table(数组指针)分配空间
int
drpc_hdlr_init(void)
{
	D_ALLOC_ARRAY(registry_table, NUM_DRPC_MODULES);	// 申请空间初始化
...
drpc_hdlr_register(int module_id, drpc_handler_t handler)
{
	...
	registry_table[module_id] = handler;	// 加入全局数组中
...
drpc_hdlr_process_msg(Drpc__Call *request, Drpc__Response *resp)
{
	...
	handler = drpc_hdlr_get_handler(request->module);	// 找回调句柄
	if (handler == NULL) {
