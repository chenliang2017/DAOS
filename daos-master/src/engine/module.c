 ...
 */
struct loaded_mod {
	/* library handle grabbed with dlopen(3), 指向对应动态库的句柄 */
	void			*lm_hdl;
	/* module interface looked up via dlsym(3), 模块支持的功能集 */
...
/* Track list of loaded modules, 跟踪模块链表用的头指针 */
...
dss_module_load(const char *modname)
{
	...
	handle = dlopen(name, RTLD_LAZY | RTLD_GLOBAL);  // 打开动态库
	if (handle == NULL) {
	...
	smod = (struct dss_module *)dlsym(handle, name);  // 从动态库中获取对应模块的dss_module(功能集)
	...
	d_list_add_tail(&lmod->lm_lk, &loaded_mod_list);  // 加到链表中
	dss_modules[smod->sm_mod_id] = smod;              // 加到数组中
...
dss_module_init_one(struct loaded_mod *lmod, uint64_t *mod_facs)
{
	...
	smod = lmod->lm_dss_mod;        // 模块支持的功能集
	/* initialize the module */
	rc = smod->sm_init();			// 模块初始化 
	if (rc) {
