...
dss_srv_handler(void *arg)
{
	...
	D_ASSERT(dmi != NULL);
	dmi->dmi_xs_id	= dx->dx_xs_id;		// xtream id
	dmi->dmi_tgt_id	= dx->dx_tgt_id;	// target id
	dmi->dmi_ctx_id	= -1;
	...
	if (dx->dx_comm) {
		...
		rc = crt_context_create(&dmi->dmi_ctx);		// 创建cart通信句柄
		if (rc != 0) {
...
dss_srv_init(void)
{
	...
	rc = pthread_key_create(&dss_tls_key, NULL);	// 创建线程变量
	if (rc) {
		rc = dss_abterr2der(rc);
		D_GOTO(failed, rc);
	}
	...
	/* initialize xstream-local storage */
	xstream_data.xd_dtc = dss_tls_init(DAOS_SERVER_TAG, 0, -1);  // 初始化xs_id为0的线程的所有模块的key作为线程变量
