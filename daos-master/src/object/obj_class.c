...
daos_oclass_attr_find(daos_obj_id_t oid, bool *is_priv)
{
	...
	oc = oclass_ident2cl(daos_obj_id2class(oid));  // 通过oid反向找到对应的oclass(分片+group)
	if (!oc) {
...
unsigned int
daos_oclass_grp_size(struct daos_oclass_attr *oc_attr)	// 返回分片个数
{
	...
	case DAOS_RES_REPL:
		return oc_attr->ca_rp_nr;	// 副本的个数

	case DAOS_RES_EC:
		return oc_attr->ca_ec_k + oc_attr->ca_ec_p;		// k(数据片) + p(校验片)
	}
}
...
oclass_fit_max(daos_oclass_id_t oc_id, int domain_nr, int target_nr)
{
	...
	oc = oclass_ident2cl(oc_id);	// 根据ID找具体的class
	if (!oc)
		return NULL;

	memcpy(&ca, &oc->oc_attr, sizeof(ca));	// class的属性拷贝到ca中
	if (oc_id == OC_RP_XSF) {
	...
	grp_size = daos_oclass_grp_size(&ca);	// 获取分片的数量
	if (ca.ca_grp_nr == DAOS_OBJ_GRP_MAX ||		// group的数量超过限制
	    ca.ca_grp_nr * grp_size > target_nr) {	// group的数量和分片数量的乘积超过target的数量
		/* search for the highest scalability in the allowed range */
		ca.ca_grp_nr = max(1, (target_nr / grp_size));
		oc = oclass_scale2cl(&ca);	// 找满足要求的，拥有最大group的class
		goto out;
	}
out:
...
obj_class_init(void)
{
	...
	for (i = 0; i < OC_NR; i++) {
		...
		if (oc->oc_resil == DAOS_RES_REPL) {  // 副本
