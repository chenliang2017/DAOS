...
struct obj_req_tgts {
	...
	struct daos_shard_tgt	 ort_tgts_inline[OBJ_TGT_INLINE_NR];     // 预先分配了10个
	 ...
	 * leader shard and following (ort_grp_size - 1) targets are the forward  服务端转发的时候, 每个group中的第一个target是主
	 * non-leader shards.													其余的target是从
	 * Now there is only one case for (ort_grp_nr > 1) that for object
	 * punch, all other cases with (ort_grp_nr == 1).
	 */
	struct daos_shard_tgt	*ort_shard_tgts;    // 所有taregt的链表
	uint32_t		 ort_grp_nr;				// group的数量
	/* ort_grp_size is the size of the group that is sent as forwarded
	 * shards
	 */
	uint32_t		 ort_grp_size;				// group中的副本数
...
obj_layout_create(struct dc_object *obj, bool refresh)
{
	...
	pool = dc_hdl2pool(dc_cont_hdl2pool_hdl(obj->cob_coh));  // 容器句柄反向找池句柄, 池句柄获取池属性
	if (pool == NULL) {
		D_WARN("Cannot find valid pool\n");
		D_GOTO(out, rc = -DER_NO_HDL);
	}

	map = pl_map_find(pool->dp_pool, obj->cob_md.omd_id);	// 通过pool的uuid找到pool_map
	if (map == NULL) {
	...
	obj->cob_md.omd_ver = dc_pool_get_version(pool);		// 通过pool获取当前的版本号(pool_map的版本)
	dc_pool_put(pool);
	rc = pl_obj_place(map, &obj->cob_md, NULL, &layout);	// 计算分布，待分析
	pl_map_decref(map);
	if (rc != 0) {
	...
	obj->cob_version = layout->ol_ver;	// 版本号, 来自pool_map的版本

	D_ASSERT(obj->cob_shards == NULL);
	D_ALLOC(obj->cob_shards, sizeof(struct dc_obj_layout) +
		sizeof(struct dc_obj_shard) * layout->ol_nr);  // 申请空间: 原始结构体大小 + 所有分片结构占得大小
	if (obj->cob_shards == NULL)
		D_GOTO(out, rc = -DER_NOMEM);

	obj->cob_shards_nr = layout->ol_nr;				// obj归属的target的总数
	obj->cob_grp_size = layout->ol_grp_size;		// 每个group中target的数量(副本数)
	old = obj->cob_grp_nr;				
	obj->cob_grp_nr = obj->cob_shards_nr / obj->cob_grp_size;	// group的数量, 类似ceph中的pg数
	...
	for (i = 0; i < layout->ol_nr; i++) {  // 遍历所有分片(target)
		struct dc_obj_shard *obj_shard;
...
static int
obj_init_oca(struct dc_object *obj)				// 初始化obj的oclass的属性:分片+group
{
	struct daos_oclass_attr *oca;
	bool			 priv;

	oca = daos_oclass_attr_find(obj->cob_md.omd_id, &priv);  // 通过对象id找到关联的oclass的属性:分片+group
	if (!oca)
		return -DER_INVAL;

	obj->cob_oca = *oca;  	// oclass存储在obj中
	if (daos_oclass_is_ec(oca) && !priv) {  // 纠删码 + priv为false
...
obj_grp_leader_get(struct dc_object *obj, int idx, unsigned int map_ver)
{
	...
	D_RWLOCK_RDLOCK(&obj->cob_lock);
	if (obj->cob_version == map_ver)
		rc = pl_select_leader(obj->cob_md.omd_id, // 对象id
				      idx / obj->cob_grp_size,	// group的序号
				      obj->cob_grp_size, NULL,	// 副本数
				      obj_get_shard, obj);
	D_RWLOCK_UNLOCK(&obj->cob_lock);
...
int
obj_dkey2grpidx(struct dc_object *obj, uint64_t hash, unsigned int map_ver)  // dkey哈希到指定的group上
{
	int		grp_size;
	uint64_t	grp_idx;
	...
	grp_idx = d_hash_jump(hash/*dkey的哈希值*/, obj->cob_shards_nr / grp_size/*group的数量*/);  // dkey的哈希值对group的数量进行哈希
	D_RWLOCK_UNLOCK(&obj->cob_lock);															// 落到指定的group上
		 ...
		 uint32_t *start_shard, uint32_t *grp_size)
{
	...
	grp_idx = obj_dkey2grpidx(obj, hash, map_ver);  // dkey哈希到指定的group上, dkey对group的数量哈希
	if (grp_idx < 0)
		return grp_idx;
		    ...
		    uint32_t flags, struct obj_auxi_args *obj_auxi)
{
	...
	if (shard_nr > OBJ_TGT_INLINE_NR) {  // 副本数大于10个
		...
		if (req_tgts->ort_shard_tgts == NULL) {
			D_ALLOC_ARRAY(req_tgts->ort_shard_tgts, shard_nr);  // 分配空间
			if (req_tgts->ort_shard_tgts == NULL)
				return -DER_NOMEM;
		}
	} else {  // 副本数小于等于10
		if (req_tgts->ort_shard_tgts != NULL &&
			...
			D_FREE(req_tgts->ort_shard_tgts);
		req_tgts->ort_shard_tgts = req_tgts->ort_tgts_inline;  // 指针指向内部的数组
	}
	req_tgts->ort_grp_nr = grp_nr;		// group的数量
	req_tgts->ort_grp_size = (shard_nr == shard_cnt) ? grp_size : shard_nr;  // 副本的数量
	for (i = 0; i < grp_nr; i++) {      // 写的情况下grp_nr为1, 只有punch的时候不为1
...
{
	struct daos_oclass_attr	*oca = obj_get_oca(obj);
	int			 obj_tf;	/* obj #tolerate failures, 对象容忍坏的分片数 */
	int			 cont_rf;	/* cont redun_fac, 容器的冗余参数 */
	int			 cont_tf;	/* cont #tolerate failures */
	int			 rc;

	cont_rf = dc_cont_hdl2redunfac(coh);  // 获取容器的冗余参数(允许坏几个副本??)
	if (cont_rf < 0) {
	...
	D_ASSERT(cont_tf >= 0);
	if (obj_tf < cont_tf) {  // 要求容器允许坏的分片数要大于obj允许坏的分片数
		rc = -DER_INVAL;
...
int
dc_obj_open(tse_task_t *task)	// 打开一个obj
{
	daos_obj_open_t		*args;
	struct dc_object	*obj;
	int			 rc;

	args = dc_task_get_args(task);  // task的枚举可以存储很多种类型的结构体, 当前函数中知道是daos_obj_open_t, 所以返回值直接强转类型了。
	D_ASSERTF(args != NULL, "Task Argument OPC does not match DC OPC\n");

	obj = obj_alloc();	// 申请空间, 创建obj
	if (obj == NULL)
		D_GOTO(out, rc = -DER_NOMEM);

	obj->cob_coh  = args->coh;		// 容器句柄
	obj->cob_mode = args->mode;		// 模式(读/写/...)
	...
	rc = dc_obj_fetch_md(args->oid, &obj->cob_md);	// 将对象的ID赋值给:cob_md.omd_id
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);

	rc = obj_init_oca(obj);  // 初始化obj的oclass的属性:分片+group
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);

	rc = obj_layout_create(obj, false);		// 获取obj的分布策略
	if (rc != 0)
		D_GOTO(fail_rwlock_created, rc);

	rc = dc_obj_redun_check(obj, args->coh);	// 验证一个obj允许坏的分片数是否大于容器允许坏的分片数
	if (rc != 0)	// 要求obj允许坏的分片数大于容器允许坏的分片数, 容器兜底
		D_GOTO(fail_layout_created, rc);

	rc = obj_ptr2pm_ver(obj, &obj->cob_md.omd_ver);	// obj中的版本号填充到odm_ver中, 都是来自pool_map
	if (rc)
		D_GOTO(fail_layout_created, rc);

	obj_hdl_link(obj);
	*args->oh = obj_ptr2hdl(obj);  // 位置指针转化为句柄
	obj_decref(obj);
out:
...
int
dc_obj_fetch_md(daos_obj_id_t oid, struct daos_obj_md *md)	// 将oid值赋值到md中
{
...
static int
obj_req_valid(tse_task_t *task, void *args, int opc, struct dtx_epoch *epoch /*入参为0*/,
	      uint32_t *pm_ver /*入参为0*/, struct dc_object **p_obj)
{
	uint32_t		map_ver = *pm_ver;
	struct obj_auxi_args	*obj_auxi;		// 对象的辅助信息结构体
	struct dc_object	*obj = NULL;
	daos_handle_t		oh;
	daos_handle_t		th = DAOS_HDL_INVAL;
	int			rc = 0;

	obj_auxi = tse_task_stack_push(task, sizeof(*obj_auxi));  // 在tse_task_private.dtp_buf的顶部预留指定空间给外部结构体用
	...
	case DAOS_OBJ_RPC_UPDATE: {		// 写操作
		daos_obj_update_t	*u_args = args;

		obj = obj_hdl2ptr(u_args->oh);	// 通过对象句柄找到obj
		if (obj == NULL)
			D_GOTO(out, rc = -DER_NO_HDL);
	...
	if (daos_handle_is_valid(th)) {  // 事务句柄传入的时候为NULL, 这里进不去
	...
	if (map_ver == 0) {  // 入参map_ver值为0
		rc = obj_ptr2pm_ver(obj, &map_ver);
		if (rc != 0)
			D_GOTO(out, rc);
	}

	*pm_ver = map_ver;   // pool_map的版本传出去
...
out:
	if (rc && obj)
		obj_decref(obj);
	tse_task_stack_pop(task, sizeof(*obj_auxi));  // 回收tse_task_private.dtp_buf的顶部预留的空间
	return rc;
}
...
{
	struct obj_auxi_args	*obj_auxi;  // 对象的辅助信息
	uint64_t		 dkey_hash;
	int			 rc;
	...
	rc = obj_csum_update(obj, args, obj_auxi);		// 校验相关
	if (rc) {
...
int
dc_obj_update_task(tse_task_t *task)			// 客户端写入口
{
	daos_obj_update_t	*args = dc_task_get_args(task);		// 强转指针类型
	struct dc_object	*obj = NULL;
	...
	rc = obj_req_valid(task, args, DAOS_OBJ_RPC_UPDATE, &epoch, &map_ver,
			   &obj);  // 有效性判断, map_ver出参赋值为pool_map的版本号, 从args中获取obj
	if (rc != 0)
		goto comp;

	if (daos_handle_is_valid(args->th)) {  // 事务句柄为0, 这里不成立
...
int
daos_obj_generate_oid(daos_handle_t coh, daos_obj_id_t *oid,  // 将OC_SX(obj的分布策略: 分片数+group数)写到oid的hi字段中
		      ...
		      daos_oclass_hints_t hints, uint32_t args)
{
	...
	/** select the oclass */
	poh = dc_cont_hdl2pool_hdl(coh);	// 通过容器句柄找到池的句柄
	if (daos_handle_is_inval(poh))
		return -DER_NO_HDL;

	pool = dc_hdl2pool(poh);			// 通过池的句柄获取池
	D_ASSERT(pool);

	rc = pl_map_query(pool->dp_pool, &attr);	// 获取池的属性信息
	D_ASSERT(rc == 0);
	dc_pool_put(pool);
	...
	if (cid == OC_UNKNOWN) {
		...
		rc = daos_oclass_fit_max(cid, attr.pa_domain_nr,
					 attr.pa_target_nr, &cid);	// 根据客户端选择的cid找对应的oclass(指定了obj的分布: group数量和副本/纠删码分片数)
	}
