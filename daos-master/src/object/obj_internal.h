...
struct dc_obj_layout {
	...
	unsigned int		do_open_count;   // 承载obj的所有target的数量
	struct dc_obj_shard	do_shards[0];    // 数组指针, 数组中存储所有分片(target)的结构体指针
};

/** Client stack object */
struct dc_object {
	/** link chain in the global handle hash table */
	struct d_hlink		 cob_hlink;				// 在全局链表中的位置。类似于boost的侵入式链表,
	/**
	...
	/** object open mode */
	unsigned int		 cob_mode;				// 对象的打开模式(读/写/...)
	unsigned int		 cob_version;			// 版本号, 来自pool_map的版本
	unsigned int		 cob_shards_nr;			// obj归属的target的总数: cob_grp_size * cob_grp_nr
	unsigned int		 cob_grp_size;			// group中target的数量(副本数)
	unsigned int		 cob_grp_nr;			// group的数量, 类似于ceph中的pg数
	/**
	...
	/** shard object ptrs */
	struct dc_obj_layout	*cob_shards;		// 承载obj的所有target的集合
};
...
obj_dkey2hash(daos_obj_id_t oid, daos_key_t *dkey)
{
	...
	if (daos_obj_id2feat(oid) & DAOS_OF_DKEY_UINT64)  // 直接使用dkey的数据, 不用哈希
		return *(uint64_t *)dkey->iov_buf;

	return d_hash_murmur64((unsigned char *)dkey->iov_buf,
			       dkey->iov_len, 5731);	// 对dkey数据进行哈希
}
